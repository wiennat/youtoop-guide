...
app.use(
  morgan(
    ":method :url HTTP/:http-version :status :res[content-length] :remote-addr - :remote-user",
    {
...
app.get("/search/:keyword", (req, res) => {
  ...
  filterDatasource.normalize(req.params.keyword).then(keywords => {
    storyDatasource.search(keywords).then(stories =>
      res.render("index", {
        stories,
...
app.post("/api/search", (req, res) => {
  ...
  const f = filterDatasource.normalize(keyword).then(keywords => {
    logger.debug(
      "normalized : %s > %s %s",
      keyword,
      keywords,
    ...
    storyDatasource.search(keywords).then(stories => res.send(stories));
  });
});
